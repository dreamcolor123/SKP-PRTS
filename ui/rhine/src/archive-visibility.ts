...
export class ArchiveVisibility {
  ...
  update(source: THREE.PerspectiveCamera, far: number, trackX: number, trackZ: number, extra: boolean, heightScale = 1): ArchiveCell[] {
    this.heightScale = heightScale;
    const inputs = [...source.projectionMatrix.elements, ...source.matrixWorldInverse.elements,
      source.near, source.far, far, trackX, trackZ, Number(extra), heightScale];
    ...
    const bottom = -6.5, top = 6.5 + (heightScale-1)*4.1;
  ...
  intersects(x: number, y: number, z: number) {
    ...
    this.box.max.set(x+2.8,y+4.1*this.heightScale,z+1.2);
