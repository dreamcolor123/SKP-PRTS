...
export class BootLettering {
      ...
      const letters = art.letters.map((letter) => {
        ...
        if (letter.path) {
          ...
          if ("stroke" in letter) {
            ...
            path.setAttribute("stroke-width", String(letter.stroke));
  ...
  useWebfonts() {
    ...
    for (const phrase of this.phrases) {
      // The product signature is always authored geometry, even with webfonts.
      if (phrase.weight === "Handmade") continue;
