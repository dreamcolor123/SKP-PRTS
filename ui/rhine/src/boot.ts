...
export class BootSequence {
  ...
  constructor(private stage: HTMLElement) {
    ...
    new BootLettering(this.brandLines[0], ["brand"]).setText(brandWordmark);
    ...
    for (const [selector, key, text] of [
      [".scan > span", "permission", "LOCAL INTERFACE READY"],
      [".welcome-heading", "welcome", "WELCOME TO"],
      [".welcome-database", "database", "KERNEL MANAGEMENT"],
    ] as const) new BootLettering(this.el(selector), [key]).setText(text);
    this.companyInk.forEach((el) =>
      new BootLettering(el.querySelector("span")!, ["company"]).setText(brandWordmark),
    );
  }
  ...
  update(time: number) {
    ...
    this.el(".powered").style.clipPath =
      `inset(0 ${100 * (1 - s.poweredLetters / "POWERED BY SKROOT PRO".length)}% 0 0)`;
