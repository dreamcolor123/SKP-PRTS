...
import { fileLocation, records } from "./data";
import { commonRowPeriod } from "./archive-loop";
import {
  cellKey,
  sameCell,
  selectionCell,
...
export class ArchiveScene {
  ...
  async load(assetUrl = publicAsset("assets/archive-cassette.glb")) {
    ...
    const shell = meshes.find(mesh => (mesh.material as THREE.Material).name.replace(/\.\d+$/, "") === "Frosted_Polymer")!;
    const shellBounds = new THREE.Box3().setFromObject(shell);
    this.shellHeight = new ArchiveShellHeight(shellBounds.min.y, shellBounds.max.y);
    ...
    for (const mesh of meshes) {
      ...
      this.shellHeight.register(geom, name);
    ...
    label.userData.labelOriginalY = label.position.y;
  ...
  setMode(mode: "hidden" | "archive" | "detail") {
    ...
    this.drawLabel(this.selectedRecord);
    if (!this.looping) {
      // The authored opening extracts the middle physical cassette. Section
      // order is applied when the interactive workspace takes over.
      this.selectedCell = { lane: 2, row: 12 };
      this.selectedSlot = 76;
  ...
  private rebaseCoordinates() {
    ...
    const rowPeriod = commonRowPeriod();
    const shift = {
      lane:
        Math.abs(this.selectedCell.lane) > 2048
          ...
          : 0,
      row:
        Math.abs(this.selectedCell.row) > 2048 && rowPeriod !== null
  ...
  select(index: number, navigation?: ArchiveNavigation) {
    ...
    this.setHover(null);
    this.lastInteraction = this.clock;
    const next = this.looping ? fileLocation(index).slot : 76;
  ...
  private drawLabel(index: number) {
    ...
    if (this.looping) {
      ...
      if (record) {
        c.fillStyle = "#e6e2d9"; c.fillRect(18, 28, 749, 372);
        c.fillStyle = "#171713"; c.font = "bold 64px MiSans";
        const label = Array.from(record.title); while (label.length > 1 && c.measureText(label.join("")).width > 690) label.pop();
        c.fillText(label.join("") + (label.length < Array.from(record.title).length ? "…" : ""), 22, 112);
        c.font = "40px MiSans"; c.fillText(record.status?.label ?? record.category, 22, 190, 725);
        c.font = "bold 80px MiSans"; c.fillText("NO." + String(index+1).padStart(3,"0"), 22, 340);
      }
    }
      ...
      if (!cancelled && browse && this.canBrowse()) {
        ...
        if (this.archiveDrag.active) {
          ...
          if (cell) {
            ...
            this.onSelect?.(index, cell);
            queueMicrotask(()=>this.onOpen?.(index));
          }
        }
      }
      reset();
  ...
  update(
    ...
    const working = !cinematic && this.container.closest<HTMLElement>("#stage")?.dataset.workspace === "true";
    const heightTarget = working ? WORKING_HEIGHT : 1;
    this.heightScale = cinematic || this.reduced ? heightTarget : THREE.MathUtils.lerp(this.heightScale,heightTarget,1-Math.exp(-dt*8));
    if(Math.abs(this.heightScale-heightTarget)<.0001)this.heightScale=heightTarget;
    this.shellHeight?.update(this.heightScale);
    this.motionTilt = followTilt(this.motionTilt,this.motionTarget,dt,working&&!this.reduced);
    ...
    if (!cinematic) {
      if (this.returnY !== null && this.rotation !== 0) {
        ...
        damp(
          this.lift,
          this.targetDetail
            ? INSPECTION_LIFT * this.heightScale
            : this.outgoing.some(
                  (o) =>
                    o.returnY !== null &&
    ...
    const cameraTarget = this.quickWorkspaceNavigation && working
      ...
      : this.returnY !== null
        ? this.detail
        : ease((this.lift.value - 0.4) / (INSPECTION_LIFT * this.heightScale - 0.4));
    ...
    for (let i = this.outgoing.length - 1; i >= 0; i--) {
      ...
      o.group.scale.y = this.heightScale;
      this.shellHeight?.placeLabel(o.group);
    ...
    this.model.scale.y = this.heightScale;
    this.shellHeight?.placeLabel(this.model);
    ...
    if (!cinematic && stage?.dataset.workspace === "true" && stage.dataset.browsing !== "true") {
      // The work surface remains a physical landscape face. Reframe the camera,
      // never stretch the model or flatten its perspective for controls.
      const width = this.container.clientWidth, height = this.container.clientHeight;
      const sideNavigation=width>=900||(width>=700&&height<540);
      const safeHeight = Math.max(160, height - (sideNavigation?112:184));
      const workSpan = Math.max(5.08 * height / width, 4.3 * this.heightScale * height / safeHeight);
      framing.span = THREE.MathUtils.lerp(framing.span, workSpan, detail);
      framing.detailX = width >= 1100 ? .55 : .5;
      framing.detailY = .48;
    }
    if (!cinematic) {
      const right = new THREE.Vector3()
    ...
    this.cells = fixed ? Array.from({ length: 160 }, (_, i) => poolCell(i))
      : this.visibility.update(this.camera, fog.far, trackX, entryZ + this.rail.value, this.extraCoverage, this.heightScale);
    ...
    for (const cell of this.cells) {
      ...
      this.dummy.scale.set(1,this.heightScale,1);
    ...
    for (let r = row - 5; r <= row + 5; r++) {
      if (r !== row)
        neighborTop = Math.max(neighborTop, -4.6 + field(r, lane) + 3.76 * this.heightScale);
    }
    for (const o of this.outgoing) {
      if (o.cell.lane === lane && Math.abs(o.cell.row - row) <= 5) {
        neighborTop = Math.max(neighborTop, o.group.position.y + 3.76 * this.heightScale);
      }
    }
    ...
    const focalPoint = this.model.position
      .clone()
      .add(new THREE.Vector3(0, 2 * this.heightScale, 0))
  ...
  getStats() {
    ...
    return {
      ...
      selectedLane: fileLocation(this.selectedRecord).lane,
