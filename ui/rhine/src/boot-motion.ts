...
export function bootMotion(appTime: number) {
  ...
  let auth = "";
  if (f < 363) {
    auth = typed("UI SESSION", f, 282, 295);
    if (f >= 320) auth += " : " + typed("LOCAL DEVICE", f, 321, 339);
  ...
  return {
    ...
    step,
    auth,
    access: "LOCAL INTERFACE INITIALIZE".slice(
      0,
      f < 170 ? 0 : Math.ceil(accessCounts[Math.min(17, f - 170)] * "LOCAL INTERFACE INITIALIZE".length / 26),
    ),
    accessOpacity: f >= 170 && f < 227 ? (f === 226 ? 0.25 : 1) : 0,
    logoOpacity: t >= 9.16 && t < 19.48 ? 1 : 0,
    logo: bootLogoTrack(frame),
    logoLetters: typed("SKROOT PRO", f, 232, 255),
    authOpacity: f >= 281 && f < 487 ? 1 : 0,
    brand: [0, 1, 2].map((line) => brandTrack(frame, line)),
    poweredLetters: typed("POWERED BY SKROOT PRO", f, 279, 295).length,
