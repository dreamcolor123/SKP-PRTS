...
// Live SKP columns have different lengths. Rebasing by the demo's fixed eight
// would silently change the record under a cell after a long drag.
export function commonRowPeriod(): number | null {
  const gcd = (a: number, b: number): number => b ? gcd(b, a % b) : a;
  let period = 1;
  for (let lane = 0; lane < archiveColumns.length; lane++) {
    const count = Math.max(1, columnFiles(lane).length);
    period = period / gcd(period, count) * count;
    if (period > 1_000_000) return null;
  }
  return period;
}
