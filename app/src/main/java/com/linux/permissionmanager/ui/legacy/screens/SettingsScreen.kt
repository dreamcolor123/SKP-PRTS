package com.linux.permissionmanager.ui.legacy.screens
...
import com.linux.permissionmanager.ui.legacy.components.*
import com.linux.permissionmanager.ui.legacy.theme.AppearanceTokens
import com.linux.permissionmanager.ui.legacy.theme.LocalChromeSurfaceAlpha
import com.linux.permissionmanager.ui.legacy.theme.LocalControlSurfaceAlpha
...
fun SettingsScreen(
    ...
    uiMode: String = "legacy",
    onUiModeChange: (String) -> Unit = {},
    uiModeSwitchAllowed: Boolean = true,
) {
            ...
            item {
                SegmentedGroup("界面") {
                    SingleChoiceSegmentedButtonRow(
                        ...
                        listOf("rhine" to "新版", "legacy" to "旧版").forEachIndexed { index, (mode, label) ->
                            SegmentedButton(
                                selected = uiMode == mode,
                                enabled = uiModeSwitchAllowed,
                                onClick = { if (uiMode != mode) onUiModeChange(mode) },
    ...
    if (basicDialog) ChoiceDialog("选择单项测试", basicChoices, { basicDialog = false }) { basicDialog = false; onBasicTest(it.value) }
    if (moduleDialog) ChoiceDialog("选择默认模块测试", moduleChoices, { moduleDialog = false }) { moduleDialog = false; onModuleTest(it.value) }
    if (rebootDialog) {
        AlertDialog(
